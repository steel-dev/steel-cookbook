...
Stripe Projects-only scenarios are the exception to manual environment setup. Document the `stripe projects build` path, and use `.env.example` only to record the variables generated by that build.
