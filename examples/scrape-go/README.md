...
[scrape-ts](../scrape-ts) and [scrape-py](../scrape-py) are the same direct API in TypeScript and Python, where the Python recipe writes the screenshot and PDF to disk. [scrape-rs](../scrape-rs) is the Rust version. For a full browser you drive yourself, [chromedp-go](../chromedp-go) and [rod-go](../rod-go) connect over CDP instead.
