...
[Steel + Swiftide (Rust)](../swiftide-rs) is the other Rust agent recipe. It reads pages through Steel's `scrape` endpoint instead of driving a browser, so compare the two when you choose between live DOM access and clean Markdown. [chromiumoxide-rs](../chromiumoxide-rs) is the same CDP browser without the agent layer. [genkit-go](../genkit-go) and [pydantic-ai](../pydantic-ai) are the tool-calling-agent shape in other languages. The [rig docs](https://docs.rs/rig-core) cover the `Tool` trait, multi-turn prompting, and the provider list.
