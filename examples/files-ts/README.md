...
[Credentials](../credentials-ts) for auth tokens kept out of the filesystem. [Auth context](../auth-context-ts) for cookies and storage state. [Profiles](../profiles-ts) for persistent user-data directories across runs. [Extensions](../extensions-ts) for loading unpacked Chrome extensions into a session.
